...
describe('resolveFrameCommands(F03 同一フレームの優先解決)', () => {
  ...
  it('ダッシュとバーストならダッシュ、バーストとスキルならバースト', () => {
    ...
    expect(resolveFrameCommands([{ type: 'LeftArmPressed' }, { type: 'BurstPressed' }])).toEqual([
      { type: 'BurstPressed' },
    ]);
  });
  ...
  it('同種の押下が 2 つあっても 1 つだけ残る', () => {
    expect(
      resolveFrameCommands([{ type: 'RightArmPressed' }, { type: 'RightArmPressed' }]),
    ).toEqual([{ type: 'RightArmPressed' }]);
  });
});
