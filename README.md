...
- **崖登り**: 明るい砂色の岩肌が登攀可の面(開始地点の後方にある高さ 6 m の崖、左手の高さ 20 m の崖、右奥のオーバーハング)。スティックを面に向けて押しながら走り込むか、面に向かってジャンプすると自動で取り付く。取り付き後はスティック上で登り、頂上で自動的によじ登る。スプリントボタン(「離す」)で面から離れ、ジャンプボタンで崖ジャンプ。灰色の岩・丘・柱・スロープの土台は登れない
