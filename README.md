...
- **崖登り**: 砂色の岩肌はすべて登攀可(崖、柱、スロープの土台、橋など。高さ 1.7 m 未満の段差を除く)。淡い水色の氷柱だけが登攀不可。スティックを面に向けて押しながら走り込むか、面に向かってジャンプすると自動で取り付く。取り付き後はスティック上で登り、頂上で自動的によじ登る。スプリントボタン(「離す」)で面から離れ、ジャンプボタンで崖ジャンプ
- **滑空**: 高さ 2 m 以上の空中でジャンプボタン。もう一度押すと解除
- **戦闘**: 紫の徘徊型 2 体を倒すと勝利。砂色のダミーは撃破対象外
- **攻撃スタイル**(ポーズメニュー): 格闘(既定)は攻撃ボタンで 3 段コンボ、長押しで敵へ踏み込む接近強攻撃(スタミナ 25)。銃撃は攻撃ボタンで射撃(射程 12 m)、長押しでタメ(最大 1 秒)、離すと貫通のタメ打ち(20〜60 ダメージ)

## 操作(PC 補助入力)

W/A/S/D 移動(Ctrl で歩き)、Shift ダッシュ/スプリント、Space ジャンプ、左クリック 攻撃(長押しで接近強攻撃 / タメ打ち)、E スキル、Q バースト、F インタラクト、右ドラッグ カメラ、ホイール ズーム、Esc ポーズ。
